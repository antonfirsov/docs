...
This summer we had six amazing interns that joined the team to work on
.NET. Their projects ranged from internal tools, over shipping
components to designing forward looking aspects:
...
> I love the near constant time operations, the huge number of use cases,
> the cleanliness! Although the dictionary has a wide variety of uses,
> there are times when I want to add multiple values per key and Dictionary
> just doesn’t quite cut it. In those situations the solution is simple:
> just build a `Dictionary<TKey, List<TValue>>`!

If you're interested to learn more about the dictionary Ian worked on,
take a look the blog posts he wrote over the summer:
...
Charles has been working on a cool Visual Studio extension called the
.NET Portability Analyzer. As developers need to target more and more
platforms this tool can be a big help in analyzing how portable your
.NET code is. It gives you a quick overview of the changes that you
would need to make in order to be able to port your code to a given
platform.  
