...
This example is simple and only shows three simple columns mapped directly on properties, but note that much more customization is possible. The `Items` property is an `IQueryable` that allows the QuickGrid to improve performance and the user experience, like being able to sort columns easily, and so much more. One of the differences between `IQueryable` and `IEnumerable` is that `IQueryable` will retrieve all the rows of the tables and apply the WHERE clause in memory but `IQueryable` will apply the WHERE clause in the source and only retrieve the requested rows.
