...
The preview bits are available [on NuGet](to-do), and also as part of [ASP.NET Core 2.2 Preview 3 (to-do)](https://blogs.msdn.microsoft.com/webdev/2018/10/17/asp-net-core-2-2-0-preview3-now-available/) and the [.NET Core SDK 2.2 Preview 3 (to-do)](https://blogs.msdn.microsoft.com/dotnet/2018/10/17/announcing-net-core-2-2-preview-3/), also releasing today. If you are working on an application based on ASP.NET Core, we recommend you upgrade to ASP.NET Core 2.2 Preview 3.
...
private static void Main(string[] args)
{
     ...
     using (var context = new MyDbContext())
     {
         ...
         context.Add(
             ...
             {
                 Name = "Bill",
                 Location = geometryFactory.CreatePoint(new Coordinate(-122.129797, 47.640049))
             });
         context.Add(
             new Friend
             {
                 Name = "Steve",
                 Location = geometryFactory.CreatePoint(new Coordinate(-122.030420, 37.331770))
             });
         context.SaveChanges();
     }
     ...
     using (var context = new MyDbContext())
     {
         var myLocation = geometryFactory.CreatePoint(new Coordinate(-122.084198, 37.422394));
...
- **Cosmos DB Provider:** Although we have made a lot of progress setting up the required infrastructure for document-oriented database support in EF Core, and have been steadily adding functionality to the provider, realistically we cannot arrive to a state in which we can release the provider with adequate functionality and quality in the current timeframe for 2.2.
  ...
  Overall, we have found that the work necessary to complete the provider to be more than we initially estimated (we maintain a checklist, including open issues at https://github.com/aspnet/EntityFrameworkCore/issues/12086). Also, ongoing evolution in Cosmos DB is leading us to frequently revisit decisions about such things as how we use the Cosmos DB SDK, whether we map all entities to a single collection by default, etc.
...
Besides these changes to the roadmap, our team has started working on the our next major release, EF Core 3.0, which will include, among other improvements, a significant overhaul of our LINQ implementation.
...
## Please try new features and EF Core with existing providers

We still have some time to fix bugs and make adjustments before EF Core 2.2 RTM, but we need your help finding outstanding issues and things that need adjusting.

Besides the new features, you can help with trying EF Core 2.2 preview 3 on applications that are using third party providers. Although we have setup our own testing for this, there might be unexpected compatibility problems, and the earlier we can detect them, the higher chances we have of addressing them before RTM.

We thank you in advance for posting any feedback to [our issue tracker](https://github.com/aspnet/EntityFrameworkCore/issues/new).
