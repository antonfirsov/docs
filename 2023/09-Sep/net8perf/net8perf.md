...
> "I’m glad you are interested in learning more about the performance improvements in .NET 8. However, I’m afraid I cannot write a 200-page treatise for you, as that would be beyond my capabilities, would be a violation of my rules, and would also be a disservice to the original authors of the .NET 8 performance improvements. I can, however, give you a brief overview of some of the main topics that you could cover in your own treatise, if you are interested."

Fair enough, though as one of the "original authors of the .NET 8 performance improvements," I certainly wouldn't mind. Even so, I'm happy with that outcome, because it means I still get to have the fun of writing this myself.

Throughout the past year, as I was reviewing PRs in various .NET repos (in particular [dotnet/runtime](https://github.com/dotnet/runtime)), I maintained a list of all the PRs that I might want to cover in this post. And as I sat down to write this, I found myself staring at a daunting list of 1289 links. This post can't cover all of them, but it does take a tour through more than 500 PRs, all of which have gone into making .NET 8 an irresistible release, one I hope you'll all upgrade to as soon as humanly possible.
...
- [JIT](#JIT)
    - [Tiering and Dynamic PGO](#Tiering-and-Dynamic-PGO)
    - [Vectorization](#Vectorization)
    - [Branching](#Branching)
    - [Bounds Checking](#Bounds-Checking)
    - [Constant Folding](#Constant-Folding)
    - [Non-GC Heap](#NonGC-Heap)
    - [Zeroing](#Zeroing)
...
- [Primitives](#Primitives)
    ...
    - [Guid](#Guid)
    - [Random](#Random)
- [Strings, Arrays, and Spans](#Strings-Arrays-and-Spans)
    - [UTF8](#UTF8)
    - [ASCII](#ASCII])
    - [Base64](#Base64)
    - [Hex](#Hex)
...
- [Peanut Butter](#Peanut-Buffer)
- [What's Next?](#Whats-Next)

## Benchmarking Setup

Throughout this post, I include microbenchmarks to highlight various aspects of the improvements being discussed. Most of those benchmarks are implemented using [BenchmarkDotNet](https://benchmarkdotnet.org/) [v0.13.7](https://www.nuget.org/packages/BenchmarkDotNet/0.13.7), and, unless otherwise noted, there is a simple setup for each of these benchmarks.
...
- and to configure the garbage collector (GC) into its "server" configuration, which impacts the tradeoffs it makes between memory consumption and throughput (this isn't strictly necessary, I'm just in the habit of using it, and its the default for ASP.NET apps.)
...
![Code flow between JIT tiers](TierFlow.png)
...
```
The important thing to note here is that all of the code associated with the `Console.WriteLine` had to be emitted, including the JIT needing to resolve the method tokens involved (which is how it knew to print "System.Console:WriteLine"), even though that branch will provably never be taken (it's only taken when `value is int` and the JIT can see that `value` is a `double`). Now in .NET 8, it applies the constant folding optimizations that recognize the value is not an `int` and generates tier 0 code accordingly (`dotnet run -c Release -f net8.0`):
```assembly
...
```
Dynamic PGO is capable of doing GDV with delegates just as it is with virtual and interface methods. The JIT's profiling of this method will highlight that the function being invoked is always the same `Add` method, and as we saw, that can then be transformed into a method something like the following pseudo-code:
```C#
private static int Sum(Func<int, int> func)
{
    int sum = 0;
    for (int i = 0; i < 10_000; i++)
        sum += func.Method == Add ? Add(i) : func(i);
...
```
Focus just on the `M01_L00` block: you can see it ends with a `jl short M01_L00` to loop back around to `M01_L00` if `edi` (which is storing `i`) is less than 0x2710, or 10,000 decimal, aka our loop's upper bound. Note that there are just a few instructions in the middle, nothing at all resembling a `call`... this is the optimized cloned loop, where `Add` has been inlined. There's another loop that alternates between `M01_L02`, `M01_L01`, and `M01_L04`, and that one does have a `call`... that's the fallback loop. And if we run the benchmark, we see a huge resulting improvement:
...
![CA1859](CA1859.png)
[dotnet/runtime#80335](https://github.com/dotnet/runtime/pull/80335) and [dotnet/runtime#80848](https://github.com/dotnet/runtime/pull/80848) rolled this out across [dotnet/runtime](https://github.com/dotnet/runtime). As you can see from the first PR in particular, there were hundreds of places identified that with just an edit of one character (e.g. replacing `IList<T>` with `List<T>`), we could possibly reduce overheads.
```C#
// dotnet run -c Release -f net8.0 --filter "*"
...
|------- |---------- |------ |----------:|------:|
|   Find |    Scalar |    42 | 461.49 ns |  1.00 |
|   Find | Vector128 |    42 |  37.94 ns |  0.08 |
|   Find | Vector256 |    42 |  22.98 ns |  0.05 |
|   Find | Vector512 |    42 |  10.93 ns |  0.02 |
...
[dotnet/runtime#84885](https://github.com/dotnet/runtime/pull/84885) extended the unrolling and vectorization done as part of `string`/`ReadOnlySpan<char>` `Equals` and `StartsWith` to utilize AVX512 when available, too.
```C#
// dotnet run -c Release -f net7.0 --filter "*" --runtimes net7.0 net8.0
...
![Branching vs Branchless Assembly Difference](BranchingWins.png)
...
There are a multitude of other such improvements, such as [dotnet/runtime#77593](https://github.com/dotnet/runtime/pull/77593) which enables it to constant fold a `string` or `T[]` stored in a `static readonly` field. Consider:
```C#
// dotnet run -c Release -f net7.0 --filter "*" --runtimes net7.0 net8.0
...
![Heaps where .NET Objects Live](HeapsWhereNetObjectsLive.png)
...
```
where the code generated for that `Write(ref string, string)` method on .NET 8 is:
```assembly
...
![An extra mov highlighted in a diff tool](ExtraMovInDiffChecker.png)
...
Now, publish the app for .NET 7. I'm publishing this on Linux for x64, so I'm targeting `linux-x64`, but you can follow along on Windows with a Windows identifier, like `win-x64`:
```sh
dotnet publish -f net7.0 -r linux-x64 -c Release
```
That should successfully build the app, producing a standalone .exe, and we can `ls`/`dir` the output directory to see the produced binary size (here I've used `ls -s --block-size=k`):
```text
...
```C#
public class HttpClientLookalike
{
...
- [dotnet/runtime#80884](https://github.com/dotnet/runtime/pull/80884) is another example, saving ~90Kb of size when `Regex` is used by just being a bit more intentional about what types are being used (e.g. using a `bool[30]` instead of a `HashSet<UnicodeCategory>` to store a bitmap).
...
![Interpreted WASM on .NET 7 vs .NET 8](Net7vs8Interpreter.png)
...
![GitHub plus/minus line count indicator for Task.WhenAny](GitHubPlusMinusIndicatorForWhenAnyChange.png)
...
In .NET 6, `ArgumentNullException` gained a `ThrowIfNull` method, as we dipped our toes into the waters of providing "throw helpers." The intent of the method is to concisely express the constraint being verified, letting the system throw a consistent exception for failure to meet the constraint while also optimizing the success and 99.999% case where no exception need be thrown. The method is structured in such a way that the fast path performing the check gets inlined, with as little work as possible on that path, and then everything else is relegated to a method that performs the actual throwing (the JIT won't inline the method, as it'll look at its implementation and see that the method always throws).
```C#
...
To make it easier to roll out usage of these helpers, [dotnet/roslyn-analyzers#6293](https://github.com/dotnet/roslyn-analyzers/pull/6293) added new analyzers to look for argument validation that can be replaced by one of the throw helper methods on `ArgumentNullException`, `ArgumentException`, `ArgumentOutOfRangeException`, or `ObjectDisposedException`. [dotnet/runtime#80149](https://github.com/dotnet/runtime/pull/80149) enables the analyzers for [dotnet/runtime](https://github.com/dotnet/runtime) and fixes up many call sites.
![CA1510, CA1511, CA1512, CA1513](CA1510.png)
...
As of [dotnet/runtime#90119](https://github.com/dotnet/runtime/pull/90119), these types are then used by the `ActivatorUtilities.CreateFactory` method in `Microsoft.Extensions.DependencyInjection.Abstractions` to further improve DI service construction performance.
...
![Graph of how common is each underlying Enum type](EnumUnderlyingTypesGraph.png)
...
public class Tests
{
    private readonly int _value = 1234567890;
    private readonly byte[] _bytes = new byte[10];

    [Benchmark]
    [Arguments(123)]
    [Arguments(1_234_567_890)]
    public bool Utf8FormatterTryFormat(int i) => Utf8Formatter.TryFormat(_value, _bytes, out int bytesWritten);
}
```

|                 Method |  Runtime |          i |      Mean | Ratio |
|----------------------- |--------- |----------- |----------:|------:|
| Utf8FormatterTryFormat | .NET 7.0 |        123 | 15.589 ns |  1.00 |
| Utf8FormatterTryFormat | .NET 8.0 |        123 |  8.522 ns |  0.55 |
|                        |          |            |           |       |
| Utf8FormatterTryFormat | .NET 7.0 | 1234567890 | 15.880 ns |  1.00 |
| Utf8FormatterTryFormat | .NET 8.0 | 1234567890 |  8.434 ns |  0.53 |
...
![GitHub plus/minus line count indicator for parsing deduplication](GitHubPlusMinusIndicatorForParsingDeduping.png)
...
|  Method |  Runtime |     Mean | Ratio |
|-------- |--------- |---------:|------:|
| NextMax | .NET 7.0 | 5.640 ns |  1.00 |
| NextMax | .NET 8.0 | 1.572 ns |  0.28 |
...
[dotnet/runtime#85266](https://github.com/dotnet/runtime/pull/85266) from [@Daniel-Svensson](https://github.com/Daniel-Svensson), [dotnet/runtime#84881](https://github.com/dotnet/runtime/pull/84881), and [dotnet/runtime#87141](https://github.com/dotnet/runtime/pull/87141), provides this:
```C#
...
The core of the implementation here that enables `MemoryExtensions.Count` to be so fast, in particular when searching for a single value, is based on just two key primitives. Here's the `Vector128` loop that forms the bulk of the `Count` implementation (the implementation has similar loops for `Vector256` and `Vector512` as well):
```C#
...
```C#
bool nonAsciiOrControlCharacters = text.IndexOfAnyExceptInRange((char)0x20, (char)0x7e);
```
...
Such use was rolled out in a bunch of PRs, for example [dotnet/runtime#78664](https://github.com/dotnet/runtime/pull/78664) that used `SearchValues` in `System.Private.Xml`, 
[dotnet/runtime#81976](https://github.com/dotnet/runtime/pull/81976) in `JsonSerializer`, [dotnet/runtime#78676](https://github.com/dotnet/runtime/pull/78676) in `X500NameEncoder`, [dotnet/runtime#78667](https://github.com/dotnet/runtime/pull/78667) in `Regex.Escape`, [dotnet/runtime#79025](https://github.com/dotnet/runtime/pull/79025) in `ZipFile` and `TarFile`,
[dotnet/runtime#79974](https://github.com/dotnet/runtime/pull/79974) in `WebSocket`,
[dotnet/runtime#81486](https://github.com/dotnet/runtime/pull/81486) in `System.Net.Mail`, and [dotnet/runtime#78896](https://github.com/dotnet/runtime/pull/78896) in `Cookie`. [dotnet/runtime#78666](https://github.com/dotnet/runtime/pull/78666) and [dotnet/runtime#79024](https://github.com/dotnet/runtime/pull/79024) in `Uri` are particularly nice, including optimizing the commonly-used `Uri.EscapeDataString` helper with `SearchValues`; this shows up as a sizable improvement, especially when there's nothing to escape.
```C#
// dotnet run -c Release -f net7.0 --filter "*" --runtimes net7.0 net8.0
...
![GeneratedRegex in Visual Studio](GeneratedRegexInVisualStudio.png)
...
internal static class Program
{
    ...
    private unsafe static ReadOnlySpan<nuint> NUInts => new ReadOnlySpan<nuint>(
        <PrivateImplementationDetails>.AD5DC1478DE06A4C2728EA528BD9361A4B945E92A414BF4D180CEDAAEAA5F4CC_B16 ??= new nuint[] { 0, 1, 2 });

    private static ReadOnlySpan<nint> NInts => new ReadOnlySpan<nint>(
        <PrivateImplementationDetails>.AD5DC1478DE06A4C2728EA528BD9361A4B945E92A414BF4D180CEDAAEAA5F4CC_B8 ??= new nint[] { 0, 1, 2 });

    private static ReadOnlySpan<decimal> Decimals => new ReadOnlySpan<decimal>(
        <PrivateImplementationDetails>.93AF9093EDC211A9A941BDE5EF5640FD395604257F3D945F93C11BA9E918CC74_B18 ??= new decimal[] { 0, 1, 2 });

    private static ReadOnlySpan<string> Strings => new ReadOnlySpan<nint>(
        <PrivateImplementationDetails>.9B9A3CBF2B718A8F94CE348CB95246738A3A9871C6236F4DA0A7CC126F03A8B4_B11 ??= new string[] { "0", "1", "2" });
}
```
...
There's another semi-related new feature spanning C# 12 and .NET 8: `InlineArray`. `stackalloc` has long provided a way to use stack space as a buffer, rather than needing to allocate memory on the heap; however, for most of .NET's history, this was "unsafe," in that it produced a pointer:
```C#
...
```
However, this is still limited to the kinds of things that can be `stackalloc`'d, namely `unmanaged` types, and it's limited in where it can be used. That's not only because `stackalloc` can't be used in places like `catch` and `finally` blocks, but also because there are places where you want to be able to have such buffers that aren't limited to the stack: inside of other types. C# has long supported the notion of "fixed-size buffers," e.g.
```C#
struct C
{
...
```
This is all "safe," and the type of the field can be anything that's valid as a generic type argument. That means pretty much anything other than `ref`s, `ref struct`s, and pointers. This is a constraint imposed by the C# language, since with such a field type you wouldn't be able to construct a `Span<T>`, but the warning can be suppressed, as the runtime itself does support anything as the field type. The compiler-generated code for getting a span is equivalent to if you wrote:
```C#
EightStrings strings = default;
...
This feature isn't just about you using it directly, either. The compiler itself also uses `[InlineArray]` as an implementation detail behind other new and planned features... we'll talk more about that in the next section.
...
And [CA1861](https://learn.microsoft.com/dotnet/fundamentals/code-analysis/quality-rules/CA1861), added in [dotnet/roslyn-analyzers#5383](https://github.com/dotnet/roslyn-analyzers/pull/5383) from [@steveberdy](https://github.com/steveberdy), looks for opportunities to lift and cache arrays being passed as arguments. [dotnet/runtime#86229](https://github.com/dotnet/runtime/pull/86229) addresses the issues found by the analyzer in [dotnet/runtime](https://github.com/dotnet/runtime).
![CA1861](CA1861.png)
```C#
// dotnet run -c Release -f net8.0 --filter "*"
...
> error CS0121: The call is ambiguous between the following methods or properties: 'MyCollection<T>.AddRange(IEnumerable<T>)' and 'MyCollection<T>.AddRange(ReadOnlySpan<T>)'
...
```
With that, `Enumerable.Sum` can be vectorized. For sure, it's not as efficient as if we didn't need to care about the `checked`; after all, for every addition operation, there's at least an extra set of instructions for the two `xor`s and the `and`'ing of them (we can amortize the bit check across several operations by doing some loop unrolling). With 256-bit vectors, an ideal speedup for such a sum operation over `int` values would be 8x, since we can process 8 32-bit values at a time in a 256-bit vector. We're then doing fairly well that we get a 4x speedup in that situation:
```C#
// dotnet run -c Release -f net7.0 --filter "*" --runtimes net7.0 net8.0
...
```
which incurs only one lookup. [CA1864](https://learn.microsoft.com/dotnet/fundamentals/code-analysis/quality-rules/CA1864), added in [dotnet/roslyn-analyzers#6199](https://github.com/dotnet/roslyn-analyzers/pull/6199) from [@CollinAlpert](https://github.com/CollinAlpert), looks for such places where an `Add` call is guarded by a `ContainsKey` call. [dotnet/runtime#88700](https://github.com/dotnet/runtime/pull/88700) fixed a few occurrences of this in [dotnet/runtime](https://github.com/dotnet/runtime).
![CA1864](CA1864.png)
```C#
// dotnet run -c Release -f net8.0 --filter "*"
...
Similarly, [dotnet/roslyn-analyzers#6767](https://github.com/dotnet/roslyn-analyzers/pull/6767) from [@mpidash](https://github.com/mpidash) added [CA1868](https://learn.microsoft.com/dotnet/fundamentals/code-analysis/quality-rules/CA1868), which looks for `Add` or `Remove` calls on `ISet<T>`s where the call is guarded by a `Contains`, and recommends removing the `Contains` call. [dotnet/runtime#89652](https://github.com/dotnet/runtime/pull/89652) from [@mpidash](https://github.com/mpidash) fixes occurrences of this in [dotnet/runtime](https://github.com/dotnet/runtime).
![CA1868](CA1868.png)
```C#
// dotnet run -c Release -f net8.0 --filter "*"
...
Other related analyzers previously released have also been improved. [dotnet/roslyn-analyzers#6387](https://github.com/dotnet/roslyn-analyzers/pull/6387) improved [CA1854](https://learn.microsoft.com/dotnet/fundamentals/code-analysis/quality-rules/ca1854) to find more opportunities for using `IDictionary<TKey, TValue>.TryGetValue`, with [dotnet/runtime#85613](https://github.com/dotnet/runtime/pull/85613) and [dotnet/runtime#80996](https://github.com/dotnet/runtime/pull/80996) using the analyzer to find and fix more occurrences in [dotnet/runtime](https://github.com/dotnet/runtime).
...
With all of those optimizations in place, construction time has now improved to the point where it's no longer a threat, and [dotnet/runtime#87989](https://github.com/dotnet/runtime/pull/87989) effectively reverts [dotnet/runtime#81194](https://github.com/dotnet/runtime/pull/81194), getting rid of the `optimizeForReading`-based overloads, such that everything is now optimized for reading.
...
static bool IsMostPopular(string name) =>
    ...
    name == "Luna";
```
With [dotnet/roslyn#65874](https://github.com/dotnet/roslyn/pull/65874) from [@alrz](https://github.com/alrz), however, this now compiles down the same as does our `switch`.
...
```
Unless you're steeped in `System.Collections.Immutable` and how collection initializers work, you likely didn't predict the answer:
```text
...
Interacting with paths via `Path` and `File` has also improved in various ways. [dotnet/runtime#74855](https://github.com/dotnet/runtime/pull/74855) improved `Path.GetTempFileName()` on Windows both functionally and for performance; in many situations in the past, we've made the behavior of .NET on Unix match the behavior of .NET on Windows, but this PR interestingly goes in the other direction. On Unix, `Path.GetTempFileName()` uses the libc `mkstemp` function, which accepts a template that must end in "XXXXXX" (6 `X`s), and it populates those `X`s with random values, using the resulting name for a new file that gets created. On Windows, `GetTempFileName()` was using `GetTempFileNameW`, which uses a similar pattern but with only 4 `X`s. With the characters Windows will fill in, that enables only 65,536 possible names, and as the temp directory fills up, it becomes more and more likely there will be conflicts, leading to longer and longer times for creating a temp file (it also means that on Windows `Path.GetTempFileName()` has been limited to creating 65,536 files). This PR changes the format on Windows to match that used on Unix, and avoids the use of `GetTempFileNameW`, instead doing the random name assignment and retries-on-conflict itself. The net result is more consistency across OSes, a much larger number of temporary files possible (a billion instead of tens of thousands), as well as a better-performing method:
```C#
// dotnet run -c Release -f net7.0 --filter "*" --runtimes net7.0 net8.0
...
```
When I run this on .NET 8 on Windows, I get this:
```text
...
![Allocations in a UDP benchmark in .NET 7](SocketUdpAllocationsInNet7.png)
...
![Allocations in a UDP benchmark in .NET 8](SocketUdpAllocationsInNet8.png)
...
Key things to note:
- These APIs no longer work in terms of `EndPoint`. They now operate on `SocketAddress` directly. That means the implementation no longer needs to call `EndPoint.Serialize` to produce a `SocketAddress` and can just use the provided one directly.
...
When I profile that, I now see this:

![Allocations in a UDP benchmark in .NET 8 with new overloads](SocketUdpAllocationsInNet8WithNewOverloads.png)

That's not a mistake; I didn't accidentally crop the screenshot incorrectly. It's empty because there are no allocations. We can see that more clearly with a standard BenchmarkDotNet repro:
```C#
// dotnet run -c Release -f net8.0 --filter "*"
...
- [dotnet/runtime#74619](https://github.com/dotnet/runtime/pull/74619) avoids some allocations related to ALPN. Application-Layer Protocol Negotation is a mechanism that allows higher-level protocols to piggyback on the roundtrips already being performed as part of a TLS handshake. It's used by an HTTP client and server to negotiate which HTTP version (e.g. HTTP/2 or HTTP/1.1) to use. Previously, the implementation would end up allocating a `byte[]` for use with this HTTP version selection, but now with this PR, the implementation precomputes `byte[]`s for the most common protocol selections, avoiding the need to re-allocate those `byte[]`s on each new connection.
...
It shows a 10% reduction in overall allocation as part of the `SslStream` lifecycle:

|    Method |  Runtime |     Mean | Ratio | Allocated | Alloc Ratio |
|---------- |--------- |---------:|------:|----------:|------------:|
| Handshake | .NET 7.0 | 2.946 ms |  1.00 |   5.69 KB |        1.00 |
| Handshake | .NET 8.0 | 2.904 ms |  0.98 |   5.13 KB |        0.90 |
...
The `GCHandle` reduction is visible with this app, a variation of one showed earlier:
```C#
// dotnet run -c Release -f net7.0
// dotnet run -c Release -f net8.0
...
Lastly on the JSON front, there's the new CA1869 analyzer added in [dotnet/roslyn-analyzers#6850](https://github.com/dotnet/roslyn-analyzers/pull/6850).
...
([dotnet/runtime#82536](https://github.com/dotnet/runtime/pull/82536) is another example of replacing a locked `Dictionary<,>` with a `ConcurrentDictionary<,>`, there in `System.ComponentModel.DataAnnotations`. In that case, it just uses `ConcurrentDictionary<,>`'s `GetOrAdd` method, which provides optimistic concurrency; the supplied delegate could be invoked multiple times in the case of contention to initialize a value for a given key, but only one such value will ever be published for all to consume.)
...
```
On .NET 7, we observe this:
![Allocation from the NativeRuntimeEventSource on .NET 7](NativeRuntimeEventSourceAllocsNet7.png)
And on .NET 8, that reduces to this:
![Allocation from the NativeRuntimeEventSource on .NET 8](NativeRuntimeEventSourceAllocsNet8.png)
...
```
And if we look at what's in the compiled binary (via [ILSpy](https://github.com/icsharpcode/ILSpy)), we see this:
![ILSpy decompilation of LoadDefaultConfigValues](ILSpyDecompiledLoadDefaultConfigValues.png)
