...
Lastly, F# 4.1 will also feature its language service sitting atop Roslyn Workspaces.  Roslyn Workspaces are a base layer of tooling where Analyzers, Refactorings, and other productivity features "plug in" to your codebase.  With F#'s language service using Roslyn Workspaces, F# can also "plug in" to the huge set of language innovations made in Visual Studio and offer a modern editing experience.

Let's dive into what's coming this year.

# Support for .NET Core

F# 4.1 is the version of F# that runs on .NET Core.  In fact, F# 4.1 will also be .NET Core-based.  The compiler and FSharp.Core library are a .NET Core console application and .NET Standard library, respectively.

Currently, this support is in alpha.  The RTM support for .NET Core will coincide with the official release of F# 4.1.
...
Many of the amazing new productivity features being built into Visual Studio today use Roslyn Workspaces as their way to interact with your code.
