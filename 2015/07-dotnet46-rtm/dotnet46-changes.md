...
* GDI+ reentrant paints fixed. [1075715]
...
* Inlined PInvoke doesn't misorder pushing managed frame and set the thread to preemptive mode. [1078669]
...
* WriteProcessMemoryStub and WriteProcessMemory have correct PInvoke signature. [1034223]
* Array.Empty<T> removed from mscorlib.asmmeta. [1036434]
* Fixed TLS index for current stack trace. [1037914]
...
* Current bgc state maintained for all heaps in server GC. [1038267]
* ICorProfilerCallback6 doesn't leak reference count. [1038285]
* Loading design-time metadata at debug time for adaptive apps is supported. [1038300]
...
* RyuJIT enforces bounds check in do-while loop. [1129316]
* RyuJIT enforces bounds check during integer overflow when array size is Int32.MaxValue. [1129371]
* Desktop CLR supports RsaCNG. [1129677]
...
* PInvoke return values are not getting corrupted by callsite for CORINFO_HELP_STOP_FOR_GC. [1134934]
...
* Insecure relative path "mscoree.dll" in COM object registry value removed. [1138059]
...
* MSRC#20579: fixed TTF vulnerability. [1141730]
...
* SHA-256 digests only generated when targeting framework versions that support it. [993689]
...
* Product version is correct in binaries' property sheets. [997577]
...
* Exception.Data on Windows 8 RTM downlevel includes IRestrictedErrorInfo information. [1000662]
* Calling a .NET method with optional arguments from COM through IDispatch::Invoke() using multiple AppDomains with GCHandles doesn't corrupt memory. [1003026]
...
* Fixed a deadlock condition in w3wp. [1026155]
...
* Native images generated by CrossGen for desktop have processor architecture data. [1028975]
...
* WebSockets don't allow untrusted users to mutate buffers used for native marshaling. [1033420]
...
* Triton 1.5 images are treated as IL by desktop CLR. [1084749]
* ManGen extended to produce manifests that support arm64 exclusions. [1084942]
...
* Fixed type forward from mscorlib to Registry contract in Store MT pack. [1090053]
...
* XmlDecryptionTransform doesn't throw stack overflow exception on canonical signed XML. [1091744]
...
* Reflection can be extended via .NET Core contracts by a privileged internal contract. [1097377]
* ParameterBuilder.SetConstant(null) doesn't throw argument exception for a nullable value type's default value. [1098429]
* Fixed an invalid field reference issue for short objects on workstation GC. [1099103]
...
* SQM data collection enabled as "OSControlled". [1104697]
...
* PInvoke return values are not getting corrupted when CORINFO_HELP_STOP_FOR_GC calls need to go through a jump stub. [1112559]
* System.Windows.Diagnostics.VisualDiagnostics.GetXamlSourceInfo supports non-DependencyObjects. [1116127]
* Fixed JIT32 simple value type return value corruption in synchronized methods. [1116938]
...
* IEExec/IEHost doesn't hang when using HTTPS with RC4-restricting security advisory update. [840679]
...
* Fixed mscoreei.dll shim buffer overrun bug. [886480]
...
* System.Xml.SecureStringHasher correctly implements hash code blinding. [915588]
* Improved performance of String.Format and StringBuilder.AppandFormat by avoiding object array allocations. [915876]
...
* Attempting to resolve alternate types that don't exist in Win8P doesn't cause null reference exception in WinMDExp. [924141]
...
* Removed a user mode write access violation when parsing long type names where the assembly name is present, but the type name is empty. [932336]
...
* WinMDExp fixes BadImageFormatException when loading managed WinMD with desktop references. [935749]
...
* Fixed a possible race condition between CordbProcess::DispatchRCEvent and CordbProcess::DeleteQueuedEvents. [945067]
* Fixed SerializationInfo denial of service vulnerability. [949835]
...
* Native WinMD files don't always have an assembly reference to System.Numerics.Vector. [1134767]
* Fixed the size of Vector<T>. [1020874]
* Removed Ngen element for System.Numerics.Vectors.dll. [1089661]
...
* Thread.CurrentPrincipal doesn't get updated with other logged in user's Thread.CurrentPrincipal. [995806]
