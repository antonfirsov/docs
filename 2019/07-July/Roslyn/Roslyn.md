...
Let me introduce you to the Roslyn analyzer package. This collection of analyzers provides a more verbose analysis, but it doesn’t ship with the default Visual Studio tools. It includes the FxCop Analyzers package that has all the FxCop rules that are still applicable to modern software development brought forward to our modern code analysis platform based on Roslyn. Let's go ahead and install Roslyn analyzers to be more productive and write better code faster!

To Install Roslyn analyzers as a NuGet package:
1. Assuming you’re using Visual Studio 15.8 or newer, choose the most recent version.
...
`A:` Yes. For analyzers that are installed from a NuGet package, those rules are [enforced at build time](https://docs.microsoft.com/visualstudio/code-quality/roslyn-analyzers-overview?view=vs-2019#build-errors), including during a CI build. Analyzers used in CI builds respect rule configuration from both [rule sets](https://docs.microsoft.com/visualstudio/code-quality/analyzer-rule-sets?view=vs-2019) and [.editorconfig files](https://docs.microsoft.com/visualstudio/code-quality/configure-fxcop-analyzers?view=vs-2019). Currently, the code analyzers that are built into Visual Studio are not available as a NuGet package, and so these rules are not enforceable in a CI build.
