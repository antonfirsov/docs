...
To better reflect this, we have updated the [framework names (TFMs)][net5-tfms]:
...
realize that .NET Standard can't ship very frequently, which means if a feature
...
seemingly simple idea is. We have tried our best to make it easier, but in the
...
thing to your application, but is entirely optional.
...
You can then invoke a code fix from the light bulb menu which will add a platform
guard to your code:
...
fixer which will annotate your own method (type or even assembly) to be
...
framework can always be consumed from all app models, so long you run the latest
version of them.
...
of .NET, for example, a minimal runtime without the current BCL. But that would
...
* **Reusable libraries**. If you're building reusable libraries that you plan on
  ...
  giving you a fairly large API set to work with. I would generally recommend
