...
In December 2019, the preview of the Microsoft.Data.Analysis DataFrame type was announced to make data exploration easy in .NET. Now in March 2020, we have introduced convenience APIs that leverage the Microsoft.Data.Analysis v0.3.0 of DataFrame when working with UDFs in Spark! These convenience APIs make data manipulation and analysis much more convenient and concise in .NET for Spark.
In this blog post, we’ll explore:
•	Implementation goals and details of the Microsoft.Data.Analysis convenience APIs in .NET for Spark
•	Example of using the Microsoft.Data.Analysis DataFrame versus traditional Spark DataFrames
•	Wrap up
Implementation goals and details
Goals
The main goal of this work is to introduce convenience APIs when working with UDFs in .NET for Apache Spark. 
Apache Spark streams data to UDFs in the Apache Arrow format. As explained on the Arrow website, “Apache Arrow is a cross-language development platform for in-memory data. It specifies a standardized language-independent columnar memory format for flat and hierarchical data, organized for efficient analytic operations on modern hardware. It also provides computational libraries and zero-copy streaming messaging and interprocess communication.”
Because Spark streams data to UDFs in the Arrow format, a user writing a UDF would need to understand the Arrow format, such as how to read Arrow columns, write to Arrow columns, and unwrap an Arrow RecordBatch (which is a batch of rows of columns of equal length) to manipulate the data it holds. Using Arrow can be somewhat lengthy or confusing for developers, requiring additional learning so that developers understand how to integrate it correctly in their applications. 
Our integration of Microsoft.Data.Analysis DataFrames provides support for Arrow-formatted data out-of-the-box, thus addressing our above usability concerns as developers no longer work with the Apache Arrow format directly and can instead work with data formats and code they already understand.
Details
Prior to this work, a user needed to specifically enumerate an Arrow RecordBatch to work with data in a UDF. But now, rather than enumerating a RecordBatch, our convenience APIs wrap the RecordBatch in a Microsoft.Data.Analysis DataFrame. Our wrapping doesn’t involve copying data, ensuring performance remains high as our ease of coding also improves. Once we’ve wrapped the batch, we can use the Microsoft.Data.Analysis DataFrame convenience APIs and start reading, writing, and manipulating our data.
There are a few kinds of Spark UDFs: pickling, scalar, and vector, and this work specifically applies to the latter two.
Pickling UDFs are the older, more traditional Spark UDF. They involve serialization (specifically pickle serialization) to convert data between the JVM object to a .NET for Spark-usable pickle object, resulting in quite a bit of overhead. Once a column is specified for processing, a pickling UDF will take each of its rows, apply the given functionality, and then add a new column.
By contrast, scalar and vector UDFs leverage Arrow serialization rather than pickling. By using Arrow, these UDFs can reap the benefits of an in-memory columnar format for data analysis, and the data transfer is more efficient. Our new .NET for Apache Spark convenience APIs specifically apply to scalar and vector UDFs as they leverage the Arrow format.
You’ll likely use both traditional Spark DataFrames and the new convenience APIs for Microsoft.Data.Analysis DataFrames in your programs. The traditional DataFrame will be used for the entire dataset in the parts of the Spark driver program outside of UDFs. When you create a UDF, the data in the traditional Spark DataFrame will be streamed to the UDFs in the worker machines in the Arrow format. Once inside the UDF, you’ll work with the Microsoft.Data.Analysis DataFrame.
There is currently 1 scenario in which you will not use these convenience APIs and will need to stick with the traditional DataFrames: intrinsics. As shown in VectorFunctions.intrinsics.cs, we can use hardware intrinsics in Spark DataFrames to further boost performance. However, the Microsoft.Data.Analysis DataFrame does not yet support intrinsics. For all other cases, you can choose to use the traditional Arrow-based APIs or the Microsoft.Data.Analysis DataFrame, and the latter will typically be the optimal choice with more concise, user-friendly code.  
Example
Simple Examples
...
The CountCharacters method is also implemented differently in each program. In VectorUdfs.cs, the definition is:
