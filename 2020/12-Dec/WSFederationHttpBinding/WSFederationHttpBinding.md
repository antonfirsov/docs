...
categories: .NET Core
desired_publication_date: 1/7/2021
...
This package contains the `WSFederationHttpBinding` type that is now used for both WSFederationHttp and WS2007FederationHttp scenarios. A key difference between .NET Framework WCF scenarios and .NET Core WCF scenarios is that the .NET Core doesn't use application config files for setting up WCF objects, so the binding will need to be configured in code rather than in `app.config`, as you might have previously done for .NET Framework.
