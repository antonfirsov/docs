The IEEE 754 specification defines many floating point standards: `binary16`, `binary32`, `binary64` and `binary128`. Most developers are familiar with `binary32`(equivalent to `float`) and `binary64`(equivalent to `double`). They provide a standard format to represent a wide range of values with a precision acceptable for many applications. .NET has always had `float` and `double` and with .NET 5 Preview 7, we've added a new `Half` type (equivalent to `binary16`)!
