...
This package contains the `WSFederationHttpBinding` type that is used to create both WSFederationHttp bindings and WS2007FederationHttp bindings. A key difference between .NET Framework WCF scenarios and .NET Core WCF scenarios is that the .NET Core doesn't use application config files for setting up WCF objects, so the binding will need to be configured in code rather than in `app.config`, as you might have previously done for .NET Framework. Note that in the current preview, the `WSFederationHttpBinding` class is incorrectly named `WsFederationHttpBinding`. This will be fixed in a future preview to have correct casing (the 'WS' should be capitalized). This blog post refers to the type by its correct name.
