...
This package contains the `WSFederationHttpBinding` type that is used to create both WSFederationHttp bindings and WS2007FederationHttp bindings. A key difference between .NET Framework WCF scenarios and .NET Standard WCF scenarios is that the .NET Standard packages don't use app config files for setting up WCF objects, so the binding will need to be configured in code rather than in app.config, as you might have previously done for .NET Framework. Note that in the current preview, the `WSFederationHttpBinding` class is incorrectly named `WsFederationHttpBinding`. This will be fixed in a future preview to have correct casing (the 'WS' should be capitalized). This blog post refers to the type by its correct name.
...
The `WSFederationHttpBinding` for .NET Standard is in preview, so the API is still evolving. For example, in the future, we intend to add [static helper methods](https://github.com/dotnet/wcf/issues/4315) on `WSFederationHttpBinding` for creating WS2007Federation and WSFederation versions of the binding so that users won't have to know which message security version is needed for the two scenarios. Another example of a planned change is the previously mentioned fix to make sure that the 'WS' prefix is correctly capitalized in all type names.
