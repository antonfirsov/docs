---
post_title: Performance Improvements in .NET 7
post_slug: performance_improvements_in_net_7
username: stoub@microsoft.com
author1: toub
microsoft_alias: stoub
featured_image: RocketPack.png
categories: .NET, Performance
summary: .NET 7 is fast. Really fast. In this post, we'll deep-dive into literally hundreds of performance improvements that contributed to that reality.
desired_publication_date: 2022-08-31
---
...
(Oh, and please don't print this to paper. "Print to PDF" tells me it would take a third of a ream.)
...
- [Interop](#Interop)
- [Threading](#Threading)
- [Primitive Types and Numerics](#PrimitiveTypesandNumerics)
- [Arrays, Strings, and Spans](#ArraysStringsandSpans)
- [Regex](#Regex)
...
Also, my standard caveat:
These are microbenchmarks. It is expected that different hardware, different versions of operating systems, and the way in which the wind is currently blowing can affect the numbers involved. Your mileage may vary.
...
```
We can see in the G_M000_IG02 block that it's doing the type check on the `IPrinter` instance and jumping to G_M000_IG05 if the check fails (`mov rcx, 0x7FFC3CD42D98`, `cmp qword ptr [rsi], rcx`, `jne SHORT G_M000_IG05`), otherwise falling through to G_M000_IG03 which is a tight fast-path loop that's inlined `BlankPrinter.Print` with no type checks in sight!
...
![Comments on GitHub PR about performance test suite improvements](AutomatedPerfResults.png)
...
- [dotnet/runtime#58196](https://github.com/dotnet/runtime/pull/58196) from [@benjamin-hodgson](https://github.com/benjamin-hodgson). Given an expression like `(byte)x | (byte)y`, that can be morphed into `(byte)(x | y)`, which can optimize away some `mov`s.
...
```console
    Directory: C:\Users\stoub\Desktop\nativeaotexample\bin\Release\net7.0\win-x64\publish
...
![Time to first UI (ms)](BlazorTimeToFirstUI.png)
...
-	Such porting is significantly easier when there's a tool to help automate it. [dotnet/runtime#72819](https://github.com/dotnet/runtime/pull/72819) enables the analyzer and fixer for performing these transformations.
![LibraryImport analyzer and fixer](LibraryImportFixer.png)
...
![Many lines of code remove diff on GitHub 1](EnumerableSumAveragePrDiff.png)

![Many lines of code remove diff on GitHub 2](EnumerableMinMaxPrDiff.png)
...
![DFA from NonBacktracking engine](DfaConstruction.png)
...
![Brotli size by level](BrotliSize.png)
...
![Brotli time by level](BrotliTime.png)
...
![Boolean array allocations in XmlSerializer in .NET 6](XmlDeserialization_Net6.png)
...
![Boolean array allocations in XmlSerializer in .NET 7](XmlDeserialization_Net7.png)
...
![Cryptography assemblies in .NET 6](Net6CryptoAssemblies.png)
...
![Cryptography assemblies in .NET 7](Net7CryptoAssemblies.png)
...
![TypeForwardedTo attributes in cryptography assembly](Net7CryptoFacades.png)
...
![Fixer for analyzer CA1854 in action](CA1854.png)
...
![GeneratedRegexAttribute analyzer](ConvertToGeneratedRegex.png)
