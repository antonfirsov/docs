...
* One of the things that really helped throughout the migration is Azure Profiler (a service that collects performance traces at runtime on Azure VMs). We deployed our nightly builds to test beds, used [wrk2](https://github.com/giltene/wrk2) as a load agent to test the scenarios under stress and collected Azure Profiler traces. These traces would then inform us of the next tweak necessary to extract peak performance from our application.
...
* After code complete, we deployed the .NET Core build to a __single__ production machine in a scale unit. A scale unit is a load balanced pool of machines.
	...
	*  All ~100 machines in this scale unit receive the exact type and amount of traffic. Then, we compared status codes, error rates, functional scenarios and performance of the __single__ machine to the remaining 99 machines to detect anomalies. 
...
Our .NET Core journey will not stop after porting. For the future, we are looking at:
