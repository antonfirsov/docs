...
We are also aiming to land full FSI support and Visual Studio tooling support for F# on .NET Core in the .NET Standard/.NET Core 2.0 timeframe.
...
Visual Studio 2017 ships with significant changes in the Visual F# Tools.
...
There are a number of known issues which are either already fixed and pending a release, or are on the immediate roadmap for getting fixed.  You can see this list [here](FIXME-BY-ADDING-LINK).  Our highest priority is to address these issues.  You can also see the [VS 2017 Updates milestone](https://github.com/Microsoft/visualfsharp/milestone/9) to track bugs, new features, and improvements which are planned to ship in a future VS 2017 update.  Items which are closed have already been completed.
...
**TODO:** instructions and link for VSIX.
