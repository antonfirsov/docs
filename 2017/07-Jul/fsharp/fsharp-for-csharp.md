...
Before we dive into concepts, let's look into a few key things where F# differs from C#.  For example, here is some basic F# code with two functions and a printed result:
...
printfn "Sum of the first 5 squares is %d" (sumOfSquares 5)
```

Note that there are no type definitions, semicolons, or braces.  The only parentheses are used to call `sumOfSquares` with `5` as input, printing its result.  The pipeline operator (`|>`) is used much like a unix-style pipe.  `square` is a function passed directly as a parameter to `List.map`.  And so on!
...
It's worth noting that everything on the left column is possible in F# and quite easy to accomplish.  There are also things on the right column which are possible in C#, though they're more difficult to accomplish.  It's also worth noting that items in the left column are not "bad" for F#, either.  Objects with methods are perfectly valid to use in F#, and can often be the best approach for F# depending on your scenario.
...
In the previous statement, the value of `1` is _bound_ to the name `x`.  `x` now always refers to the value `1` for its lifetime, and cannot be modified.  For example, the following code does not reassign the value of `x`:

```fsharp
let x = 1
x = x + 1 // Doesn't compile!
```

Instead, the second line is an equality comparison to see if `x` is equal to `x + 1`.  In fact, there is no way to express reassignment of `x` unless it is explicitly defined to be `mutable`.  Although you could then reassign the value (with the `<-` operator), you'll quickly find that it's easier to think about how to solve problems _without_ reassigning values.  This allows you to play to the strengths of F#.
...
As mentioned earlier, F# makes use of *expressions*.  This is in contrast with C#, which uses _statements_ for nearly everything.  The difference between the two can initially seem subtle, but there is one thing to always keep in mind: an expression produces a value.  Statements do not.
...
Diving a bit deeper, F# actually uses expressions to model statements.  These return the [`unit`](https://docs.microsoft.com/dotnet/fsharp/language-reference/unit-type) type.  `unit` is roughly analogous to `void` in C#, although it's subtly different:

```fsharp
let names = [ "Alf"; "Vasily"; "Shreyans"; "Jin Sun"; "Moulaye" ]

// For loop.  The 'do' keyword enforces that the inner scope should have type 'unit'.
// If the inner scope doesn't, its result is implicitly ignored.
...
F# arrays, lists, and sequences also have array, list, and sequence expression syntax.  This is very convenient for different scenarios where you can generate one programmatically.
...
If you are familiar with LINQ methods, the following table should help you understand analogous functions in F#.  This table is, in no way, intended to be exhaustive.
...
Additionally, there is the [Tour of F#](https://docs.microsoft.com/dotnet/fsharp/tour) document in our official documentation for .NET and its languages.
